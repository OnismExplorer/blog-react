...
const ArticleList: React.FC<ArticleListProps> = ({articleList}) => {
    ...
    return (
        <div className="font-optima max-w-4xl mx-auto space-y-10">
            {/* 标题栏 */}
            <div
                className="text-greyFont border-b border-dashed border-b-lightGray pb-[5px] mb-[50px] items-center flex space-x-1">
                <svg viewBox="0 0 1024 1024" width="20" height="20" style={{verticalAlign: "-2px"}}>
                    <path
                ...
                <span className="text-base font-optima font-bold text-lime-500">发现</span>
            </div>

            {/* 文章列表 - 网格布局 */}
            <div className="md:grid md:grid-cols-2 md:gap-3 space-y-8 md:space-y-0">
                {articleList.map((article, idx) => {
                    ...
                    // 图片是否在左边
                    const isImageLeft = rowIndex % 2 === 0 ? isOdd : !isOdd;
                    const containerClass = clsx(
                        'relative select-none cursor-pointer overflow-hidden rounded-[10px] mb-10 shadow-article transition-all duration-300 ease-[ease] hover:shadow-article-hover animate-hideToShow',
                        {
                            'flex-row-reverse': isImageLeft,
                            'flex-row': !isImageLeft,
                            'md:flex': true,
                            'block': true,
                            'md:h-[310px]': true, // 桌面端固定高度300px
                            'h-[470px]': true, // 移动端高度450px
                        },
                        {
                            'animate-slide-bottom': isOdd,
                            'animate-slide-top': !isOdd,
                        }
                    );
                    // 文章封面路径
                    const imageSrc = !common.isEmpty(article.articleCover)
                    ...
                    const imageWrapper = (
                        <div className={clsx(
                            "w-full h-[225px] md:w-1/2 md:h-full relative overflow-hidden", // 添加overflow-hidden确保图片放大不会溢出
                            {
                                "md:absolute md:left-0": !isImageLeft,
                                "md:absolute md:right-0": isImageLeft
                            }
                        )}>
                            {!hasError ? (
                                <Image
                                    ...
                                    draggable={false}
                                    lazy
                                    className="transition-transform duration-1000 hover:scale-110 transform-origin-center" // 添加transform-origin-center确保从中心放大
                                    onError={() => setHasError(true)}
                                />
                            ) : (
                                <div title={article.articleTitle}
                                     className="w-full h-full flex justify-center items-center bg-lightGreen">
                                    <div
                                        className="font-optima font-bold text-lg leading-[1.8] tracking-[8px] text-white text-center md:line-clamp-none">
                                        <div>
                                            {"遇事不决"}
                                            <span className="md:hidden">，</span>{/*移动端添加逗号*/}
                    ...
                    return (
                        ...
                        >
                            ...
                            {/*内容*/}
                            <div className={clsx(
                                'w-full font-custom md:w-1/2 md:h-full px-5 py-5 max-md:pb-8 max-md:pt-5 flex flex-col relative',
                                {
                                    'md:absolute md:left-0': isImageLeft,
                                    'md:absolute md:right-0': !isImageLeft
                                }
                                ...
                                <div
                                    className="font-optima font-bold flex flex-wrap text-base lg:text-sm text-greyFont mb-2 items-center">
                                    <svg viewBox="0 0 1024 1024" width="14" height="14" className="mr-1">
                                ...
                                >
                                    <h3 className=" text-lg lg:py-1 font-bold max-md:text-base truncate">{article.articleTitle}</h3>
                                </Tooltip>
                                <div
                                    className="w-full flex text-greyFont text-sm lg:text-[12px] font-optima mb-2 max-md:mb-2 space-x-5 lg:space-x-[5px]">
                <span className="font-optima font-bold flex items-center gap-1 lg:gap-0.5">
                  <svg viewBox="0 0 1024 1024" width="14" height="14" style={{verticalAlign: "-2px"}}>
              ...
              <path
                  ...
                  fill="#FFFFFF"></path>
                    ...
                    {article.viewCount} 热度
                                    ...
                                    <span className="font-bold flex items-center gap-1 lg:gap-0.5">
                  <svg viewBox="0 0 1024 1024" width="14" height="14" style={{verticalAlign: "-2px"}}>
              <path
              ...
              <path
                  ...
                  fill="#885F44"></path>
                                ...
                                <div className="relative">
                                    <p className="text-xm leading-[1.7] overflow-hidden text-ellipsis line-clamp-4 max-sm:line-clamp-3 mb-4">
                                        {article.articleContent || "暂无内容"}
                                        {"..."}
                                    </p>
                                </div>

                                <div className={clsx('flex flex-row gap-2 mt-10 max-md:my-2 absolute bottom-3 max-md:bottom-2', {
                                    'left-5': !isOdd,
                                    'right-5': isOdd,
                                })}>
                                    {/* 分类标签 */}
                                    <span
                                        className={clsx('py-[3px] text-center px-[10px] lg:px-[2px] bg-maxLightGray rounded text-sm text-greyFont transition-all duration-300 cursor-pointer hover:bg-themeBackground hover:text-white ',{
                                            'md:ml-3 md:mr-1':isOdd
                                        })}
                                        onClick={(e) => {
                                            e.stopPropagation();
                                            navigate(`/sort?sortId=${article.sortId}`);
                                        }}
                                    >
                                    <svg viewBox="0 0 1024 1024" width="15" height="15"
                                         className="inline-block align-[-3px] mr-1 lx-2">
                                        <path
                                    ...
                                    {/* 标签 */}
                                    <span
                                        className={clsx('py-[3px] px-[10px] lg:px-[2px] text-center bg-maxLightGray rounded text-sm text-greyFont transition-all duration-300 cursor-pointer hover:bg-violet-600 hover:text-white',{
                                            'md:mr-3 md:ml-1':!isOdd
                                        })}
                                        onClick={(e) => {
                                            e.stopPropagation();
                                            navigate(`/sort?sortId=${article.sortId}&labelId=${article.labelId}`);
                                        }}
                                    >
                                    <svg viewBox="0 0 1024 1024" width="15" height="15"
